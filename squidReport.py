...
def countStatus(filter_content):
    ...
    for data in filter_content:
        if (filter_content[data]["status"] in status):
            status[filter_content[data]["status"]] = status.get(
                filter_content[data]["status"]) + 1
        else:
    ...
    for a in status:
        x.add_row(
            [a,
             status.get(a),
             "{0:.2f}%".format(status.get(a) * 100 / len(filter_content))
             ])
...
def countRemoteHost(filter_content):
    ...
    for data in filter_content:
        if (filter_content[data]["remote_host"] in host):
            host[filter_content[data]["remote_host"]] = host.get(
                filter_content[data]["remote_host"]) + 1
            bandwidth[filter_content[data]["remote_host"]] += int(
                filter_content[data]["bytes"])
    ...
    for a in host:
        x.add_row(
            [a,
             host.get(a),
             "{0:.2f}%".format(host.get(a) * 100 / len(filter_content)),
             "{0:.2f} M".format((bandwidth.get(a) / 1024) / 1024)
             ])
...
def countFileType(filter_content):
    ...
    for data in filter_content:
        if (filter_content[data]["file_type"] in filetype):
            filetype[filter_content[data]["file_type"]] = filetype.get(
                filter_content[data]["file_type"]) + 1
        else:
    ...
    for a in filetype:
        x.add_row(
            [a,
             filetype.get(a),
             "{0:.2f}%".format(filetype.get(a) * 100 / len(filter_content))
             ])
